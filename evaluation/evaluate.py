...
Metrics collected per episode:
  - success       : True only if the episode reaches the full evaluation horizon
                    without collision or early termination
  - non_collision : True if no collision occurred, even if the episode ended early
  - collision     : True/False, propagated from info["collision"]
...
def run_evaluation(
    ...
    for ep in range(n_episodes):
        ...
        # Task success:
        # The episode is successful only if the agent survives the full
        # evaluation horizon without collision. Early endings without collision
        # are treated as failures, because they usually correspond to lost line,
        # stuck behaviour, or another premature termination.
        success = (not collision) and (steps >= max_steps_per_ep)

        # Non-collision is kept as a separate metric.
        # This tells us whether the agent avoided collisions, even if it failed
        # to complete the full task.
        non_collision = not collision

        if collision:
            termination_reason = "collision"
        elif steps >= max_steps_per_ep:
            termination_reason = "max_steps"
        else:
            termination_reason = "early_end"
    ...
    non_collision_rate = float(np.mean([r["non_collision"] for r in results])) * 100
    ...
    termination_counts = Counter(r["termination_reason"] for r in results)
    ...
    summary = {
        ...
        "non_collision_rate_pct": round(non_collision_rate, 2),
    ...
    print(f"  Avg lap steps:     {avg_lap_steps:.0f}" if not np.isnan(
        avg_lap_steps) else "  Avg lap steps:     N/A (no successes)")
